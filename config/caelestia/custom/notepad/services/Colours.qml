...
Singleton {
    ...
    //
    // Driven from here rather than statically in notepad.lua so it follows the
    // transparency toggle instead of needing a compositor reload.
    function reloadHyprRules(): void {
        const rule = ns => `hl.layer_rule({ match = { namespace = "notepad" }, ${ns} })`;
