...
## 9. What this means for the notepad

The notepad is a panel **inside this shell**, not a separate Quickshell config. It was
standalone at first (`qs -p ~/.config/caelestia/custom/notepad`), and the reason it moved
is the single most important consequence of §6:

> A merged panel has **no internal edge**, so no shadow is drawn at the seam.

A separate Wayland surface can never join caelestia's silhouette. Caelestia's border
casts its `MultiEffect` shadow inward into the hole, and a translucent surface sitting on
top of that shadow transmits ~15% of it as a dark band along its bottom edge — visible in
proportion to how bright the wallpaper is behind it, which is why it is easy to
mis-measure as absent over a dark patch. No layer rule fixes this: Hyprland 0.56.2 has no
`xray` *layerrule* (only `decoration:blur:xray` and `misc:session_lock_xray` exist in the
binary), and drawing a second translucent pass over the border composites to `1-(1-a)²`
— 0.98 at a=0.85 — which is worse.

In-process, the notepad is just another `PanelBg` in the one `BlobGroup`, so it gets:

- **No seam.** It is part of the silhouette that gets shadowed, so nothing is drawn
  between it and the border.
- **Real merging.** Open it with the launcher and the two fuse via `smin` into one
  continuous mass (§1). `Panels.qml` stacks them the way caelestia chains its right-edge
  panels, so their *contents* don't overlap while their *blobs* do.
- **`deformMatrix` plumbing**, `wallLuminance`-corrected colours, the shared input mask,
  the focus grab, and the transparency/blur rules — all for free, because they are
  properties of the window it now lives in.

What it still carries of its own: `modules/notepad/services/` (Store, Clip, Dict, Files,
Picker, Projects, Emoji) and three components that caelestia has no equivalent of
(`EditMenu`, `EmptyState`, `SearchBox`). Everything else — `Anim`, `CAnim`, `StyledRect`,
`StyledText`, `StateLayer`, `MaterialIcon`, `Colours` — comes from caelestia directly.

**Name collision worth remembering:** Qt 6.10 added `QtQuick.Controls.SearchField`, and
an explicit module import beats the implicit same-directory one. A file here called
`SearchField.qml` silently resolves to Qt's instead, which has no `hint` property. Hence
`SearchBox.qml`.

### Measuring the bottom edge

If a band is ever suspected again, two traps produced convincing false positives:

- **The wallpaper cycles.** Comparing an "open" capture against a "closed" capture taken
  minutes earlier attributes a wallpaper feature to the panel. Capture the pair back to
  back.
...
To separate geometry from compositing in one shot, force the blob layer `opacity: 1`.
Opaque, a correctly-positioned panel reads a uniform surface colour right down to the
border with no gap.
